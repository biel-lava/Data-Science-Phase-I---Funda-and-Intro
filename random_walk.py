...
class RandomWalk:
    ...
    def fill_walk(self):
        while len(self.x_values) < self.num_points:
            ...
            x_distance = choice([0, 1, 2, 3, 4, 5, 6, 7, 8]) # choose the distance of the step
            ...
            y_distance = choice([0, 1, 2, 3, 4, 5, 6, 7, 8]) # choose the distance of the step
...
# Mk 2 random_walk
# adds the get_step method to simplify fill_walk method
...
class RandomWalk:
    ...
    def fill_walk(self):
        while len(self.x_values) < self.num_points:
            ...
            # Update the x-value of the point
            x_step = self.get_step() # use the method to calculate for the step
            x = self.x_values[-1] + x_step # update the x-value of the current point
            self.x_values.append(x) # add the x-value into the list

            # Update the y-value of the point
            y_step = self.get_step() # use the method to calculate for the step
            y = self.y_values[-1] + y_step # update the x-value of the current point
            self.y_values.append(y) # add the x-value into the list
    
    def get_step(self): # simplifies the calculation for the step for x and y values
